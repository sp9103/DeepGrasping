...
void Solver<Dtype>::Step(int iters) {
  ...
  ///////////////////////////학습 이터레이션 기록시점. -> 인풋 데이터랑 아웃풋 데이터 어디서?////////////////////////////////////
  while (iter_ < stop_iter) {
    ...
    if (display) {
      ...
      for (int j = 0; j < result.size(); ++j) {
		...
		if (strcmp(output_name.c_str(), "concat") == 0)
			continue;
...
void SGDSolver<Dtype>::ComputeUpdateValue(int param_id, Dtype rate) {
  ...
  //////////////////////////////////////////////////
  vector<std::string> name = this->net_->layer_names();
  Dtype weight[32 * 32];
  //////////////////////////////////////////////////
  switch (Caffe::mode()) {
  case Caffe::CPU: {
  ...
  case Caffe::GPU: {
	...
	//////////////////////////////////////////////////////////////
	//int shape0 = net_params[param_id]->shape()[0];
	//int shape1 = 0;
	//if (net_params[param_id]->shape().size() == 1)
	//	shape1 = 1;
	//else
	//	shape1 = net_params[param_id]->shape()[1];
	//cudaMemcpy(weight, net_params[param_id]->gpu_diff(), sizeof(Dtype) * shape0 * shape1, cudaMemcpyDeviceToHost);
	////////////////////////////////////////////////////////////////
#else
    NO_GPU;
#endif
