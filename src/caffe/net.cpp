...
Dtype Net<Dtype>::ForwardFromTo(int start, int end) {
  ...
  //Training 할때 Layer[1]에서 직접 넣을수도 있음
...
void Net<Dtype>::CopyTrainedLayersFrom(const NetParameter& param) {
  ...
  for (int i = 0; i < num_source_layers; ++i) {
    ...
    for (int j = 0; j < target_blobs.size(); ++j) {
	  ...
	  //////////////////////////////////////////여기서 target_blobs를 바꿔치기하자///////////////////////////////////////////////////
	  //if (target_blobs.size() == 2){
		 // char W[32];
		 // char b[32];
		 // static int i = 0;

		 // int layerLen = strlen(source_layer_name.c_str());
		 // i = atoi(&source_layer_name.c_str()[layerLen - 1]);

		 // sprintf(W, "Matlab_uvd_xyz\\W%d.bin", i);
		 // sprintf(b, "Matlab_uvd_xyz\\b%d.bin", i);

		 // FILE *W_fp = fopen(W, "rb");
		 // FILE *b_fp = fopen(b, "rb");

		 // int W_rows, W_cols;
		 // fread(&W_rows, sizeof(int), 1, W_fp);
		 // fread(&W_cols, sizeof(int), 1, W_fp);

		 // Dtype *weight = target_blobs[0]->mutable_cpu_data();

		 // for (int j = 0; j < W_rows * W_cols; j++){
			//  double w_val;
			//  fread(&w_val, sizeof(double), 1, W_fp);
			//  weight[j] = (Dtype)w_val;
		 // }

		 // Dtype WeightCheck[32 * 32];
		 // memcpy(WeightCheck, target_blobs[0]->cpu_data(), sizeof(Dtype)*target_blobs[0]->shape()[0] * target_blobs[0]->shape()[1]);

		 // int b_rows, b_cols;
		 // fread(&b_rows, sizeof(int), 1, b_fp);
		 // fread(&b_cols, sizeof(int), 1, b_fp);

		 // Dtype *bias = target_blobs[1]->mutable_cpu_data();

		 // for (int j = 0; j < b_rows; j++){
			//  double b_val;
			//  fread(&b_val, sizeof(double), 1, b_fp);
			//  bias[j] = (Dtype)b_val;
		 // }

		 // Dtype BiasCheck[32];
		 // memcpy(BiasCheck, target_blobs[1]->cpu_data(), sizeof(Dtype)*target_blobs[1]->shape()[0]);
